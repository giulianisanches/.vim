...
function s:NewFileXML( )
    ...
    if &filetype == 'xml' || (!exists ("g:did_xhtmlcf_inits") && exists ("g:xml_use_xhtml") && (&filetype == 'html' || &filetype == 'xhtml' || &filetype == 'phtml' || &filetype == 'rhtml'))
...
function s:ParseTag( )
    ...
    execute "normal! \"xy%%"
    let ltag = @"
    if (&filetype == 'html' || &filetype == 'phtml' || &filetype == 'xhtml') && (!exists ("g:xml_no_html"))
        let html_mode = 1
